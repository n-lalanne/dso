...
void parseArgument(char* arg)
{
    ...
    if(1==sscanf(arg,"imu=%s",buf))
    {
        imuFile = buf;
        printf("loading imu from %s!\n", imuFile.c_str());
        return;
    }
...
int main( int argc, char** argv )
{
    ...
    if (!imuFile.length())
    {
        printf("ERROR: imu missing");
        exit(1);
    }
    if (!configFile.length())
    {
        printf("ERROR: config file missing");
        exit(1);
    }
    if (!groundtruthFile.length())
    {
        printf("ERROR: groundtruth file missing");
        exit(1);
    }

    dso_vi::ConfigParam configf(configFile);
    dso_vi::GroundTruthIterator groundtruth_iterator(groundtruthFile);
    dso_vi::ImuIterator imu_iterator(imuFile);

    // find the timestamp from filename
    std::vector<double> image_timestamps;
    std::vector<std::string> image_files = reader->getImageFiles();
