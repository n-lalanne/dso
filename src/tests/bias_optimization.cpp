...
int main(int argc, char **argv)
{
    ...
    // noisy biases estimates
    double bias_acc_noise = 0;
    double bias_gyro_noise = 0;
    if (argc >= 7) {
        bias_acc_noise = atof(argv[5]);
        bias_gyro_noise = atof(argv[6]);
    }
    ...
    if (is_imu_measurement) {
        PreintegratedCombinedMeasurements *preint_imu = dynamic_cast<gtsam::PreintegratedCombinedMeasurements*>(imu_preintegrated_);
        Matrix J_imu_Rt_i, J_imu_v_i, J_imu_Rt_j, J_imu_v_j, J_imu_bias_i, J_imu_bias_j;
        CombinedImuFactor *imu_factor = new CombinedImuFactor(
                X(0), V(0),
                X(1), V(1),
                B(0), B(1),
                *preint_imu
        ...
        imuBias::ConstantBias bias1_noisy(bias1.vector() + (Vector6() <<
                (rand()/(double)RAND_MAX - 0.5) * bias_acc_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_acc_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_acc_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_gyro_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_gyro_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_gyro_noise
        ).finished());

        imuBias::ConstantBias bias2_noisy(bias2.vector() + (Vector6() <<
                (rand()/(double)RAND_MAX - 0.5) * bias_acc_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_acc_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_acc_noise,
                (rand()/(double)RAND_MAX - 0.5) * bias_gyro_noise,
...
//        imuBias::ConstantBias bias1_noisy = bias1;
//        imuBias::ConstantBias bias2_noisy = bias2;

        imuBias::ConstantBias bias1_tmp = bias1_noisy;
        imuBias::ConstantBias bias2_tmp = bias2_noisy;
        Eigen::Matrix<double, 15, 15> information_matrix = preint_imu->preintMeasCov().inverse();

        NavState prop_state = imu_preintegrated_->predict(prev_state, bias1);

        std::cout << "start: " << std::endl;
        std::cout << bias1_tmp << std::endl;
        std::cout << bias2_tmp << std::endl;
        std::cout << "--------------------" << std::endl;

        for (size_t gn_idx = 0; gn_idx < 3; gn_idx++) {
            Eigen::Matrix<double, 15, 12> J;
            ...
            initial_values.insert(X(1), actual_state.pose());
            initial_values.insert(V(0), prev_state.velocity());
            initial_values.insert(V(1), actual_state.velocity());
            initial_values.insert(B(0), bias1_tmp);
            initial_values.insert(B(1), bias2_tmp);
            imu_factor->linearize(initial_values);

            Eigen::Matrix<double, 15, 1> err = imu_factor->evaluateError(
                    prev_state.pose(), prev_state.velocity(), prop_state.pose(), prop_state.velocity(),
                    bias1_tmp, bias2_tmp,
                    J_imu_Rt_i, J_imu_v_i, J_imu_Rt_j, J_imu_v_j, J_imu_bias_i, J_imu_bias_j
            );

            J.leftCols<6>() = J_imu_bias_i;
            J.rightCols<6>() = J_imu_bias_j;

            H = J.transpose() * information_matrix * J;
            b = J.transpose() * information_matrix * err;
            inc = H.ldlt().solve(-b);

            bias1_tmp = imuBias::ConstantBias(bias1_tmp.vector() + inc.head<6>());
            bias2_tmp = imuBias::ConstantBias(bias2_tmp.vector() + inc.tail<6>());

            std::cout << "Iter: "   << gn_idx << std::endl;
            std::cout << "err: "    << err.transpose() << std::endl;
            std::cout << "bias1: "  << bias1_tmp << std::endl;
            std::cout << "bias2: "  << bias2_tmp << std::endl;
            std::cout << "--------------------" << std::endl;
        }
    }
