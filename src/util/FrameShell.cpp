...
gtsam::NavState FrameShell::PredictPose(gtsam::NavState ref_pose_imu, double lastTimestamp)
{
    ...
    std::cout << "In prediction:\nVelocity norm of the previous pose: " << (last_frame->navstate.velocity()).norm() <<" GT: " << ( last_frame->groundtruth.velocity).norm() << std::endl;
...
//                ref_pose_imu.velocity()
//        );
        predicted_pose_imu = ref_pose_imu;
    }
//    if(false)
//    {
