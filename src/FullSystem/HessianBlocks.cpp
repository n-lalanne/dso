...
void FrameHessian::setStateZero(const Vec10 &state_zero)
{
	...
	for(int i=0;i<6;i++)
	{
		...
		SE3 b2w_rightEps_P_x0 = (get_worldToImu_evalPT() * EepsP) * get_worldToImu_evalPT().inverse();
		SE3 b2w_rightEps_M_x0 = (get_worldToImu_evalPT() * EepsM) * get_worldToImu_evalPT().inverse();
		nullspaces_pose.col(i) = (b2w_rightEps_P_x0.log() - b2w_rightEps_M_x0.log())/(2e-3);
	}
	//nullspaces_pose.topRows<3>() *= SCALE_XI_TRANS_INVERSE;
	//nullspaces_pose.bottomRows<3>() *= SCALE_XI_ROT_INVERSE;

	// scale change
	SE3 b2w_rightEps_P_x0 = (get_imuToWorld_evalPT());
	b2w_rightEps_P_x0.translation() *= 1.00001;
	b2w_rightEps_P_x0 = get_worldToImu_evalPT() * b2w_rightEps_P_x0;
	SE3 b2w_rightEps_M_x0 = (get_imuToWorld_evalPT());
	b2w_rightEps_M_x0.translation() /= 1.00001;
	b2w_rightEps_M_x0 = get_worldToImu_evalPT() * b2w_rightEps_M_x0 ;
	nullspaces_scale = (b2w_rightEps_P_x0.log() - b2w_rightEps_M_x0.log())/(2e-3);
